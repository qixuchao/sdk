...
(function () {
  ...
  var isMobile = /(iPhone|iPod|Android|ios|mobile)/i.test(ua);
  var isAndroid = /Android|Linux/.test(ua);
  var isIos = /\(i[^;]+;( U;)? CPU.+Mac OS X/gi.test(ua);
  var isWechat = /MicroMessenger/i.test(ua);
  var isQQ = /qq/i.test(ua); // 手机QQ

  var isQQBrowser = /mqqbrowseer/i.test(ua);
  var isTencentBrowser = isWechat || isQQ || isQQBrowser;
  ...
  var gdtManager;
  var GdtManager$1 = (function (config) {
    if (!gdtManager) {
  ...
  var registerQQ = (function (Union) {
    Union.register('gdt', {
      ...
      onInit: function onInit(data, _ref) {
        ...
        GdtManager$1(this.config).bindSlot(data.consumerSlotId, this.id, function (status, adInfo) {
          clearInterval(timeout);
  ...
  var Union = /*#__PURE__*/function (_Event) {
    ...
    function Union(name, options) {
      ...
      _defineProperty(_assertThisInitialized(_this), "onTimeOut", function () {
        ...
        if (_this.status === '1' || _this.status === '11') {
          _this.status = '10';
        ...
        if (this.consumerLength > 0) {
          ...
          each(this.consumers, function (con) {
            ...
            if (union) {
              ...
              union.run(con, $container, _this2.config);
            } else {
